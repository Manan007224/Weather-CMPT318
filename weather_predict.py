...
def prepare_and_clean_crime_data(CRIME_DIR) :
...
def image_extracted_features_data(IMAGE_DIR,sun_present_file,average_color_file) :
    ...
    image_data = pd.DataFrame(contents,columns=['title'])


    image_data['year'] = image_data['title'].str.extract('katkam-([0-9]{4}).*\.jpg')
    image_data['month'] = image_data['title'].str.extract('katkam-[0-9]{4}([0-9]{2}).*\.jpg')
    image_data['day'] = image_data['title'].str.extract('katkam-[0-9]{4}[0-9]{2}([0-9]{2}).*\.jpg')
    image_data['hour'] = image_data['title'].str.extract('katkam-[0-9]{4}[0-9]{2}[0-9]{2}([0-9]{2}).*\.jpg')

    image_data['timestamp'] = image_data.apply(image_datetime,axis=1)
    ...
    image_data['sun_present'] = sun_present['sun_present']
    image_data['sky_color_r'] = sky_color['r']
    image_data['sky_color_g'] = sky_color['g']
    image_data['sky_color_b'] = sky_color['b']
    image_data['color_label'] = sky_color['Label']

    return image_data

def plot_colorlabels_vs_sunpresent(image_data,visualization_dir) :
    ...
    color_data = image_data[['color_label','sun_present']]
...
def predict_unlabelled_weather(ndata,cdata,image_data,snow_data,model,labeled_data_X) :

    ## Here all the data from GHCN Weather, Image extracted data, weatherstats and crime data for unlabeled data is compiled

    wiudata = ndata.merge(image_data,on='timestamp')
...
def main() :
    ...
    CRIME_DIR = './data/crime/crime_csv_all_years.csv'
    ...
    ## CRIME DATA
    CRIME_DIR = './data/crime/crime_csv_all_years.csv'
    cdata = prepare_and_clean_crime_data(CRIME_DIR)

    ## IMAGE DATA
    IMAGE_DIR = './data/image_data/katkam-scaled'
    SUN_PRESENT_FILE = './data/image_data/SUN_PRESENT.csv'
    KATKAM_AVERAGE_COLOR_FILE = './data/image_data/KATKAM_labeled_colors2.csv'
    ...
    SNOW_FILE = './data/weatherstats/snow.csv'
    ...
    weather_labeled_data_X = D['x']
    weather_labeled_data_y = D['y']
    ...
    model_evaluations(models,weather_labeled_data_X,weather_labeled_data_y)
    ...
    ## TRAINING AGAIN
    best_model.fit(weather_labeled_data_X,weather_labeled_data_y)
    missing_labels = predict_unlabelled_weather(ndata,cdata,idata,snow_data,best_model,weather_labeled_data_X)
